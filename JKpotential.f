...
C
      ...
      WRITE(6,*) 'ITYPE:1.Plate wing'
      WRITE(6,*) 'ITYPE:2.Elliptic wing'
      WRITE(6,*) 'ITYPE:3.Arc wing'
      WRITE(6,*) 'ITYPE:4.Symmetric Joukowski wing'
      WRITE(6,*) 'ITYPE:5.Asymmetric Joukowski wing'
      READ(5,*) ITYPE
      IF(ITYPE.EQ.1) THEN
        R=1.D0
      ELSE
      ...
      ELSEIF(ITYPE.EQ.3) THEN
        WRITE(6,*) 'BETA=',DACOS(R1/R)/(2.D0*PI)*360.D0
        TETA0=(0.D0,1.D0)*DSQRT(R**2-R1**2)
        BETA=DACOS(R1/R)
      ...
      ELSEIF(ITYPE.EQ.5) THEN
        ...
        TETA0=(1.D0,0.D0)*(R1-R*DCOS(BETA))
...
C
  ...
  110 CONTINUE
     ...
     &    +(0.D0,1.D0)*R*RAMDA(J)*DSIN(THETA(I)+ALPHA)
        ...
        Z(I,J)=(ZETA(I,J)+R1**2/ZETA(I,J))
        DZ(I,J)=AE*(1.D0-R1**2/ZETA(I,J)**2)
        X(I,J)=DREAL(Z(I,J))
        Y(I,J)=DIMAG(Z(I,J))
        POR(I,J)=DREAL(F(I,J))
        PHR(I,J)=DIMAG(F(I,J))
